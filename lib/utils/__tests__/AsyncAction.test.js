...
describe('AsyncAction', () => {
  test('Resolve.', async () => {
    const asyncAction = new _AsyncAction.AsyncAction(() => Promise.resolve('Success'));
    await expect(asyncAction.execute()).resolves.toBe('Success');
  });
  test('Retying 3 times will increment retries to 3', async () => {
    const asyncAction = new _AsyncAction.AsyncAction(() => new Promise((resolve, reject) => {
  ...
  test('Fail and retry thrice then resolve on 3rd retry', async () => {
    const asyncAction = new _AsyncAction.AsyncAction(async () => {
    ...
    await expect(asyncAction.setRecoverStrategy(3, false).execute()).resolves.toBe(success);
  });
  test('Fail and retry thrice then reject on 3rd retry', async () => {
    ...
    await expect(asyncAction.setRecoverStrategy(3, false).execute()).rejects.toThrow('Failed');
  });
  test('Cancel an execution and test onCancel defaults', async () => {
    const asyncAction = new _AsyncAction.AsyncAction(() => new Promise(resolve => setTimeout(resolve, 5000)));
    ...
    expect(onCancel.mock.calls[0][0]).toBeInstanceOf(_AsyncAction.CancelledError);
  ...
  test('Cant cancel a resolved/rejected execution', async () => {
    const resolveAsyncAction = new _AsyncAction.AsyncAction(() => Promise.resolve(success));
    const rejectAsyncAction = new _AsyncAction.AsyncAction(() => Promise.reject(failure));
    const onCancel = jest.fn();
    ...
    expect(onCancel).toHaveBeenCalledTimes(0); // Have to catch the error or it will fail our test.
  ...
  test('Reject 1000 times and timeout in 0ms.', async () => {
    ...
    const asyncAction = new _AsyncAction.AsyncAction(action);
    await expect(asyncAction.setRecoverStrategy(1000, false).timeoutIn(0).execute()).rejects.toThrow('Timed Out');
  });
  test('Override retry strategy.', async () => {
    ...
    const asyncAction = new _AsyncAction.AsyncAction(action);
    await expect(asyncAction.setRecoverStrategy(error => {
      return error.message === 'Failed';
    }, false).execute()).resolves.toBe('Success');
    expect(count).toBe(4);
  });
  ...
  test('Retry action after a failure.', () => {
    ...
    const asyncAction = new _AsyncAction.AsyncAction(action);
    return asyncAction.execute().catch(error => {
